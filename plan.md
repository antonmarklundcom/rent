...
8. **Same-model chaining (AFK mode)**: when this phase's PR is merged green and its exit checklist fully passes, check the phase table in the header — if the NEXT phase uses the SAME model, open its prompt file from `prompts/` and continue it immediately in this same session, exactly as if freshly prompted. **Before chaining, run a pre-chain audit on the phase just merged**: re-run build + all verify scripts/tests, re-read your own merged diff adversarially (what would a reviewer or CI reject? what edge case is untested?), and fix anything found in a follow-up commit to the next phase's branch — a defect merged now silently poisons every later phase, and this is the last cheap moment to catch it. Spend real effort here; it is part of the phase, not overhead. Do NOT chain when: the next phase uses the other model (stop and report), any §4.4 stop-condition arose (stop and ask), or the current phase could not fully meet its exit bar (finish it first). If a session ends mid-chain, re-pasting the unfinished phase's prompt in a fresh window resumes safely — phases are re-runnable by design (§4.6).
